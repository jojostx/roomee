...
class UpdateProfilePage extends Component implements HasForms
{
    ...
    protected function getVerificationStatus(Get $get): VerificationStatus
    {
        $status = $this->getFormModel()?->verification_status;

        return $status instanceof VerificationStatus
            ? $status
    ...
    protected function getVerificationDescription(Get $get): string
    {
        return match ($this->getVerificationStatus($get)) {
            ...
            VerificationStatus::PENDING => 'Your identity documents are under review.',
            VerificationStatus::REJECTED => 'Your previous verification request was rejected. Please upload new documents to resubmit.',
    ...
    {
        $isVerified = $this->getVerificationStatus($get) === VerificationStatus::APPROVED;
            ...
            Section::make(fn (Get $get): HtmlString => $this->getIdentityVerificationHeading($get))
                ...
                ->schema([
                    Placeholder::make('verification_rejection_reason')
                        ...
                        ->visible(fn (Get $get): bool => $this->getVerificationStatus($get) === VerificationStatus::REJECTED && filled($get('rejection_reason'))),
