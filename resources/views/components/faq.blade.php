...
@foreach($QandA as $questionGroup => $questions)
    <div class="grid grid-cols-1 md:grid-cols-6" id="q_{{ $loop->iteration }}">
        <div class="px-2 py-4 border border-b-0 border-secondary-700 md:px-4 md:border-b col-span-full md:col-span-2 md:border-r-0">
            <P class="text-lg font-bold text-primary-700 lg:text-xl">{{ $questionGroup }}</P>
        </div>
        <div class="border-b border-secondary-700 col-span-full md:col-span-4">
            @foreach($questions as $question => $answer)   
                <div>
                    ...
                    <label class="relative flex items-center justify-between px-4 py-4 border border-b-0 border-secondary-700 cursor-pointer md:px-6 accordion-label hover:bg-primary-300" for="{{ $question }}">
                        <p>{{ $question }}</p>
                    ...
                    <div class="p-8 text-sm border-l border-r border-secondary-700 accordion-content">{{ $answer }}</div>
                </div>               
            @endforeach 
        </div>               
    </div>
