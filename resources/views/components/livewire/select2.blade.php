...
<div x-data="{{ $name }}_dropdown()" class="flex flex-col md:mt-0 md:col-span-1" x-on:click.away="show = false" x-on:keydown.escape.window="show = false">
    ...
    <div class="relative">
        <ul x-cloak class="dropdown_ul" x-show="show" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" x-ref="{{ $name }}_dropdown" aria-expanded="false">
            ...
            <li tabindex="0" x-on:keydown.arrow-up.prevent="previousUp($event)" x-on:keydown.arrow-down.prevent="nextDown($event)" x-on:keydown.enter="toggleCheckbox($refs.{{ $option['name'] }}_tag, $refs.{{ $option['name'] }}_checkbox)" class="flex flex-row-reverse items-center justify-between mb-1 border border-transparent rounded-md hover:bg-gray-200 focus:border-gray-400 focus:bg-gray-100 focus:outline-none">
                <x-livewire.checkbox type="checkbox" id="{{ $option['name'] }}" wire:model="{{ 'selected'.$slot }}" name="{{ $name }}[]" value="{{$option['id']}}" x-ref="{{ $option['name'] }}_checkbox" :checked="in_array($option['id'], $selectedOptions)?'true':''" class="hidden {{ $name }}_checkboxes" autocomplete="off" />
                <label for="{{ $option['name'] }}" class="flex items-center justify-between w-full h-full px-2 py-2 rounded-md labels_{{ $name }}">
                    {{ ucfirst($option['name']) }}
        ...
        <div class="relative ">
            @foreach ($options as $option)
            <div x-ref="{{ $option['name'] }}_tag" class="{{ $name }}_tags {{ in_array($option['id'], $selectedOptions)?'inline-flex':'hidden' }} items-center mb-2 pl-2 bg-gray-300 rounded-md">
                <p class="pl-1 pointer-events-none">{{ ucfirst($option['name']) }}</p>
                <label tabindex="0" x-on:keydown.enter="uncheck($refs.{{ $option['name'] }}_tag, $refs.{{ $option['name'] }}_checkbox)" for="{{ $option['name'] }}" id="label_{{ $option['name'] }}" class="flex items-center px-2 py-2 cursor-pointer {{ $name }}_labels">
