    ...
    <x-page-header>
        <div class="flex items-center justify-between w-full">
            ...
            <div class="flex items-center overflow-hidden text-xs text-blue-800 border border-blue-800 rounded-md sm:text-sm">
                <button wire:click="switchPage" class="px-1 sm:px-2 py-1 @if ($currentPage=='recieved') bg-blue-200 @endif border-r border-blue-800">Recieved requests</button>
                <button wire:click="switchPage" class="px-1 sm:px-2 py-1 @if ($currentPage=='sent') bg-blue-200 @endif hover:text-blue-600">Sent requests</button>
            </div>
        </div>
    </x-page-header>
    @if ($currentPage == 'sent')
        ...
        @else
            <div class="flex flex-wrap px-2 mx-auto max-w-7xl sm:px-6 lg:px-8">
                @foreach ($sentRequests as $user)
                    @livewire('cards.requests.sent', ['user' => $user], key($user->id))
                @endforeach
            </div>
        @endif
    @else
        ...
        @else
            <div class="flex flex-wrap px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
                @foreach ($recievedRequests as $user)
                    @livewire('cards.requests.recieved', ['user' => $user], key($user->id))
                @endforeach
            </div>
        @endif
    @endif
